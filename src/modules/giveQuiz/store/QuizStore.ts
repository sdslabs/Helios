...
const useQuizStore = create<quizStore>((set) => ({
 ...
 nextQuestion: () => set((state) => {
    if (state.currentQuestionIndex >= state.sections[state.currentSectionIndex-1].questions.length ) {
      ...
      return { 
        ...
        currentQuestion: state.sections[state.currentSectionIndex-1].questions[state.currentQuestionIndex], // And here
      };
    }
   }),
