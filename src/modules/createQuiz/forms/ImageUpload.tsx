...
const ImageUpload = ({ onImageUpload, initialImage }: ImageUploadProps) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data } = await axiosInstance.post('/images/upload-direct', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });

      // Get the pre-signed URL from the backend response
      const { imageUrl } = data;

      // Set the image preview and call onImageUpload with the imageUrl
